...
A failing check that would break a release (the workflow's check job rejects
it, or the artifacts come out wrong) is marked blocks_release / 🛑.
...
CONVENTIONS = 'https://github.com/jejezz/application-release-templates/blob/main/conventions'
PREFIX = 'art.zoomon'
HOLDER = 'Jongyun Ahn'
...
# A failing check with one of these prefixes stops a release: the release
# workflow's check job would reject it, or the published artifacts would be
# wrong. Everything else is convention debt — fix it, but it doesn't block.
BLOCKING = ('identity.no-com-example', 'identity.app-identity', 'identity.product-name',
            'version.', 'workflow.', 'packaging.iss', 'packaging.linux-install', 'icons.linux',
            'l10n.parity', 'readme.check', 'release.')
...
class Audit:
    ...
    def add(self, id: str, area: str, status: str, message: str, fix: str = '', doc: str = '') -> None:
        blocks = status == 'fail' and id.startswith(BLOCKING)
        self.checks.append(Check(id, area, status, message, fix, f'{CONVENTIONS}/{doc}' if doc else '', blocks))
    ...
    def identity(self) -> None:
        ...
        if any(v.startswith('com.example') for v in ids.values()):
            ...
            self.add('identity.no-com-example', a, 'fail', f'default identifier left: {bad}',
                     f'set every identifier to {PREFIX}.<name> (before the first release)', doc)
        elif ids:
        ...
        if 'macos' in self.platforms:
            if self.product_name():
                ...
                self.add('identity.product-name', a, 'fail', 'PRODUCT_NAME missing in AppInfo.xcconfig',
                         'set PRODUCT_NAME to the display name', doc)
        ...
        if not ident:
            ...
            self.add('identity.app-identity', a, 'fail', 'lib/app_identity.dart still has __PLACEHOLDERS__',
                     'fill displayName and repositoryUrl', doc)
            ...
            self.add('identity.app-identity', a, 'fail', 'AppIdentity.displayName != PRODUCT_NAME',
                     'make them identical (the desktop check job compares them)', doc)
        else:
    ...
    def versioning(self) -> None:
        ...
        if tag:
            ...
            if self._semver(name) < self._semver(tag_ver):
                self.add('version.not-behind-tag', a, 'fail', f'pubspec {name} is behind latest tag {tag}',
                         'the tag was made on another branch or the bump never merged — bring main up to the tag', doc)
            else:
            ...
            if tm and build < int(tm.group(1)):
                self.add('version.build-monotonic', a, 'fail',
                         f'build {build} is lower than {tag}\'s build {tm.group(1)}', 'build number only goes up', doc)
            elif tm:
        ...
        if hard:
            self.add('version.not-hardcoded', a, 'fail', f'version hardcoded in lib/: {", ".join(sorted(set(hard)))}',
                     'read it with package_info_plus', doc)
        else:
        ...
        if cargo:
            ...
            self.add('version.cargo', a, 'pass' if ok else 'fail',
                     f'Cargo.toml {cm.group(1) if cm else "?"} vs pubspec {name}', '' if ok else 'bump together', doc)
    ...
    def workflow(self) -> None:
        ...
        if self.desktop:
            ...
            if not rel:
                self.add('workflow.desktop', a, 'fail', '.github/workflows/release.yml missing',
                         'copy desktop/.github/workflows/release.yml', doc)
            else:
                checks = {
                ...
                if missing:
                    self.add('workflow.desktop', a, 'fail', 'release.yml is an older template: missing ' + '; '.join(missing),
                             'replace with desktop/.github/workflows/release.yml (keep app-specific build steps)', doc)
                else:
        ...
        if self.mobile:
            for name, plat in (('release-ios.yml', 'ios'), ('release-android.yml', 'android')):
                ...
                text = files.get(name, '')
                if not text:
                    self.add(f'workflow.{plat}', a, 'warn', f'{name} missing', 'copy mobile/.github/workflows/', doc)
    ...
    def packaging(self) -> None:
        ...
        if 'windows' in self.platforms:
            ...
            if not iss:
                self.add('packaging.iss', a, 'fail', 'no installer/windows/*.iss', 'copy desktop/installer/windows/app.iss', doc)
            else:
                problems = []
    ...
    def icons(self) -> None:
        ...
        if unused:
            ...
            dead = [p for p in unused if p.name not in refs and p.parent.name not in refs]
            if dead:
    ...
    def localization(self) -> None:
        ...
        if 'untranslated-messages-file' not in y:
            problems.append('no untranslated-messages-file (CI check)')
        ...
        if 'ko' in keys and 'en' in keys:
            ...
            self.add('l10n.parity', a, 'fail' if diff else 'pass',
                     ...
                     'translate every key' if diff else '', doc)
        else:
    ...
    def readme(self) -> None:
        ...
        if checker.exists():
            ...
            self.add('readme.check', a, 'pass' if r.returncode == 0 else 'fail',
                     'check_readme.py ' + ('passes' if r.returncode == 0 else f'{len(errors)} errors: ' + errors[0][9:90]),
                     '' if r.returncode == 0 else 'python3 tool/readme/check_readme.py', doc)
        else:
    ...
    def release_readiness(self) -> None:
        ...
        if behind and behind != '0':
            self.add('release.up-to-date', a, 'fail', f'{behind} commits behind origin', 'git pull')
        elif ahead and ahead != '0':
            self.add('release.up-to-date', a, 'warn', f'{ahead} commits not pushed', 'push / open the bump PR')
        else:
        ...
        if tag and tag[1:] == v:
            self.add('release.version-bumped', a, 'fail', f'pubspec {v} is already released as {tag}',
                     'scripts/bump-version.sh patch|minor|major', 'versioning.md')
        elif v:
            self.add('release.version-bumped', a, 'pass', f'next release v{v} (latest {tag or "none"})')
        if self.git('tag', '--list', f'v{v}'):
            self.add('release.tag-free', a, 'fail', f'tag v{v} already exists', 'bump the version', 'tagging.md')
...
def markdown(root: Path, checks: list[Check], platforms: list[str]) -> str:
    ...
    lines = [f'# Conventions audit: {root.name}', '',
             ...
             f'❌ {counts["fail"]} (🛑 {len(blocking)} block a release) · ⚠️ {counts["warn"]} · ✅ {counts["pass"]}', '']
    area = None
    for c in sorted(checks, key=lambda c: (list(dict.fromkeys(x.area for x in checks)).index(c.area),
        ...
        mark = '🛑' if c.blocks_release else ICON[c.status]
...
def main() -> None:
    ...
    checks = audit.run()
    if args.json:
        print(json.dumps({'app': str(root), 'platforms': audit.platforms,
                          'checks': [asdict(c) for c in checks]}, ensure_ascii=False, indent=1))
    else:
        print(markdown(root, checks, audit.platforms))
