...
기본 판은 **세로 그라데이션 `#1F2A38 → #0A0E14`** 입니다 (allwinner에서
쓰는 값). 모든 앱이 같은 판을 쓰면 Dock이나 홈 화면에서 "한 제작자의
앱"으로 보입니다. 글리프 색으로 앱을 구분하고, 판 색은 바꾸지 않습니다.

- 지금은 saturn이 단색 `#142233`을 쓰고, 그 스크립트 docstring에는
  `#0A0E14`라고 적혀 있어 실제 코드와도 다릅니다. 다음 아이콘 갱신 때
  이 규칙으로 맞춥니다.
