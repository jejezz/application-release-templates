...
**첫 태그는 프리릴리스**(데스크톱): 새 앱은 기능을 만들기 전에
`v0.1.0-rc.1`을 태그해서 빌드·서명·공증·설치 프로그램이 끝까지 도는지
확인합니다. 프리릴리스 태그에서는 CI가 README를 **구조만** 검사하므로(기능
설명·스크린샷은 아직 없으니까) 이 단계에서 통과합니다. 정식 `v0.1.0`부터는
README가 완성돼야 합니다. 모바일은 README를 검사하지 않고, 스토어 버전
문자열이 숫자만 허용되므로 첫 태그도 `v0.1.0`(내부 테스트 트랙)입니다.
